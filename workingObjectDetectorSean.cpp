...
static int worthyCluster(double distAvg, int * ranges);
      ...
      const Fovea &botSaliency)
{
    ...
    while(removed == 1){
        ...
        for (counter1 = 0; counter1 < allClusters.size(); counter1++) {
            ...
            for (counter2 = 0; counter2 < allClusters[counter1].size(); counter2++) {
                Point p = allClusters[counter1][counter2];
                dist = perpDist(line, p);
                distAvg += dist;
            }
            distAvg /= allClusters[counter1].size();

            if(worthyCluster(distAvg, ranges) != 1){
                removed = 1;
    ...
    if(worthyCluster(distAvg, ranges)){
        printf("this is a worthy cluster\n");
    }else{
        printf("this cluster is unworthy\n");
    }
